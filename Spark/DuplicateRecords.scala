...
object DuplicateRecords {
  ...
  def main(args: Array[String]) {
   ...
   //   5, 9, 1, r => r.getString("person_id") + ", " + r.getString("first_name") + ", " + r.getString("last_name"))
    ...
    //  myRDD.saveAsTextFile("/resources/delta")
     
     
     val dataRDD = sc.textFile("resources/data").cache()
       
     val newRdd = sc.textFile("resources/delta")
       
     //dataRDD.union(newRdd).map(rec => (rec.split(",")(0).toInt, rec)).reduceByKey((x,y)=> y).foreach(println)
